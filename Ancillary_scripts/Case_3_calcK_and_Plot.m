...
if load_data == false
    ...
    save case_3_data_T_vec
elseif load_data == true
    load case_3_data_T_vec
end
...
for m = 1:num_N_s
    N_s = N_s_vec(m);
    obj_fun = @(x) sum((k_vec_temp-1./sqrt(Mone(T,x(1),x(2),x(3)))).^2);
    x0 = [1e-3,1,1];
    lb = [0,0,0];
    ub = [inf,1,1];
    ...
    M_fine = Mone(T_fine,t_c_fit,beta_fit,alpha_fit);
    K_model_plot = 1./sqrt(M_fine);
    
    %% plot results
    figure('units','normalized','outerposition',[0 0 1 1])
    scatter(T,k_vec_temp,'r+','LineWidth',1.5)
    set(gca, 'XScale', 'log')
    hold on
    semilogx(T_fine,K_model_plot,'r','LineWidth',1)
    % show asymptotes
    ...
    grid minor
    title (['Case 3 - $g_{1d}(\tau) = \exp\left(-\left(\frac{\tau}{\tau_c}\right)^1\right)$ - $\tau_c$ = ',...
